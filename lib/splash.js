...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe }){
  let div = dom('.splash',
    ...
    dom('form id=invite',
      ...
      // Hi-Ether: START
      dom('p',
          `You must hold at least ${process.env.MINIMUM_BALANCE} ${process.env.TOKEN_NAME} (`,
          dom('strong', process.env.NETWORK_NAME),
          `)  to become a member of ${process.env.TEAM_NAME}.`
      ),
      // Hi-Ether: END
    ...
    `),
    dom('script src=https://cdnjs.cloudflare.com/ajax/libs/socket.io/1.4.4/socket.io.min.js'),
